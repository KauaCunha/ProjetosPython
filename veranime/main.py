...
if __name__ == "__main__":
    ...
    if len(anime_list) == 0:
        ...
        print(f"Found {len(anime_list)} results: ") # Mostra a quantidade de anime encontrados
        # Enumera os anime e os nomes deles
        for (i, anime) in enumerate(anime_list):
            anime_title = anime["title"]
            print(f"{i+1}. {anime_title}")
		
		# Escolha de anime ---------------------------------------------------------------
		# Escolha com base na numeração dos animes
        ...
        if len(episode_list) == 0:
            ...
            # Aqui eu adicionei uma forma pra que ele pesquise direto no navegador
            newt=2
            urlt=episode_url
            webbrowser.open(urlt)#,newt=newt)
